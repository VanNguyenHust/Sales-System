...
export function nextFocusableNode(node: HTMLElement, filter?: Filter): HTMLElement | Element | null {
  const allFocusableElements = Array.from(document.querySelectorAll(FOCUSABLE_SELECTOR));
...
export type MouseUpBlurHandler = (event: React.MouseEvent<HTMLAnchorElement | HTMLButtonElement>) => void;
export const handleMouseUpByBlurring: MouseUpBlurHandler = ({ currentTarget }) => currentTarget.blur();

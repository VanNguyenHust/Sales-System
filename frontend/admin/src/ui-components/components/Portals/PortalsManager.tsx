...
import { PortalsContainerElement, PortalsManagerContext } from "./PortalsManagerContext";
...
export function PortalsManager({ children, container }: PortalsManagerProps) {
  const [portalContainerElement, setPortalContainerElement] = useState<PortalsContainerElement>(null);

  const currentContainer = container ?? portalContainerElement;
  const contextValue = useMemo(() => ({ container: currentContainer }), [currentContainer]);

import type { HSBAColor, HSBColor, HSBLAColor, HSLAColor, HSLColor, RGBAColor, RGBColor } from "./types";
...
export function hsbToRgb(color: HSBAColor | HSBColor): RGBAColor {
...
export function rgbToHsb(color: RGBColor | RGBAColor): HSBAColor {
...
export function rgbToHsl(color: RGBColor | RGBAColor): HSLAColor {
  const { hue, saturation: rawSaturation, lightness: rawLightness, alpha = 1 } = rgbToHsbl(color as RGBAColor, "l");
...
function roundNumberToDecimalPlaces(value: number, decimals: number) {

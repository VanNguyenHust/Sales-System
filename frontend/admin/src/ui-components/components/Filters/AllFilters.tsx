...
import { FilterGroupInterface, FilterInterface, FilterPrimaryAction } from "./types";
...
export const AllFilters = ({
  ...
  const searchFilterMarkup = !hideSearchFilter ? (
    <StyledFilterSearch>
      <TextField
  ...
  const primaryActionMarkup = primaryAction ? (
    <Button
      ...
      onClick={() => {
        ...
        const { autoCloseSheet = true } = primaryAction;
        if (autoCloseSheet) {
          dismissToggle();
        }
      }}
  ...
  return (
    <Sheet open={open} onClose={dismissToggle}>
      <StyledWrapper>
        ...
        {searchFilterMarkup}
        <StyledBodyWrapper hideSearchFilter={hideSearchFilter} {...boundary.props}>
          <StyledScrollable>
...
const StyledScrollable = styled(Scrollable)`
  padding: ${(p) => p.theme.spacing(0, 2, 4)};
  flex: 1 1 0;
  border-bottom: ${(p) => p.theme.shape.borderDivider};

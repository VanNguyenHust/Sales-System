...
export function measureColumn(tableData: TableMeasurements) {
  return (column: HTMLElement, index: number): ColumnVisibilityData => {
...
function isEdgeVisible(position: number, start: number, end: number, edgeType: string) {
...
export function getRowClientHeights(rows: NodeList | undefined) {

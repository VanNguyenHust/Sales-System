import { sanitize } from "app/components/RichTextEditor/sanitize";
...
export const toNormalText = (str: string, joinWithChar?: string) => {
  DefaultDiacriticsRemovalMap.forEach((item) => {
    str = str.replace(item.letters, item.base);
  });
  if (joinWithChar) {
    str = str.replace(/ +/g, joinWithChar);
  }
  return str;
};

export const htmlToNormalText = (str: string) => {
  if (!str) return "";
  const div = document.createElement("div");
  div.innerHTML = sanitize(str);
